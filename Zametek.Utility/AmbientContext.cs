...
namespace Zametek.Utility
{
    ...
    {
        private static ConcurrentDictionary<Type, AsyncLocal<byte[]>> s_State = new ConcurrentDictionary<Type, AsyncLocal<byte[]>>();
        ...
        public static void SetData<T>(T data) where T : class
        {
            ...
            s_State.GetOrAdd(typeof(T), _ => new AsyncLocal<byte[]>()).Value = Serialize(data);
        }
        ...
        {
            Debug.Assert(IsDataContract(typeof(T)) || typeof(T).IsSerializable);
            if (s_State.TryGetValue(typeof(T), out AsyncLocal<byte[]> data))
            {
                if (data.Value == null)
                {
                    return null;
        ...
        {
            s_State.TryRemove(typeof(T), out AsyncLocal<byte[]> data);
        }
